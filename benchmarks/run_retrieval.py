#!/usr/bin/env python3
"""Retrieval benchmark runner for BrainCore's hybrid retrieval.
...
----------------------
The retrieval benchmark evaluates post-extraction behavior, so it needs
known preserve-schema rows even on a fresh clone. To make the runner
reproducible, this module auto-loads ``benchmarks/seed_smoke.sql`` when
...
under a synthetic ``braincore_lib`` package name, matching the pattern
used in ``examples/mcp_server/server.py``.
"""
...
def run(no_seed: bool = False, force_seed: bool = False) -> dict[str, Any]:
    ...
    if relevance_at_10 <= 0:
        raise AssertionError(
            "Retrieval benchmark produced zero relevance hits. "
            "Check benchmark tenant, seed data, and retrieval filters."
        )
