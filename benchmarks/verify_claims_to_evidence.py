...
#   public phrasing "13-table preserve schema" / "14-table preserve
...
# "4-stream hybrid retrieval" (a source-file presence check) correctly
# live in this tier.
#
...
CLAIM_BINDERS: dict[str, ClaimBinder] = {
    ...
    # row (expected: 13) and the post-008 production row (expected: 14).
    "13-table preserve schema": _preserve_tables_binder(),
    "14-table preserve schema": _preserve_tables_binder(),
    "7 deterministic parsers": _count_binder("deterministic\\s+parsers?", "parsers"),
    ...
    "evidence grounding rate": _percent_binder(r"grounding"),
...
# class identified in Codex Pass 3. The same regex (e.g. "N facts") is
...
# "26,947 facts in the production corpus" binds ONLY to the production
...
def _framing_anchor_satisfied(
    ...
    a smoke clause and a production clause sit close together (e.g.
    "13-table smoke schema. Our production deployment has 14 tables.").
    ...
    Codex Pass 3 even when the README places both phrasings in close
...
def verify_claim(
    ...
    ``json``. This closes Codex Pass 3 Gap 2 ("numeric structural rows
    not README-bound").
    ...
    if source_type == "sql_query":
        ...
        # (the benchmark runners and Stream A migration tests already
        # validated the count). Codex Pass 3 Gap 2 fix: when the row
        ...
        if expected is None:
            return True, (
                ...
                f"validated by Stream A migration tests)"
            )
        if readme_text is not None:
            ok, err = _bind_readme_to_actual(
    ...
    if source_type == "file_lines":
        ...
        # Codex Pass 3 Gap 2 fix: file_lines rows are now README-value-
    ...
    if source_type == "literal":
        # Codex Pass 3 Gap 4 fix: implement the documented `literal`
...
def _run_self_test() -> int:
    ...
    # Case A: README cites "26,947 facts" inside a "production corpus"
    ...
    readme_citing_facts = (
        "Our production corpus has 26,947 facts as of 2026-04-09."
    )
    with tempfile.TemporaryDirectory() as tmp:
        repo = Path(tmp)
        p, f, msgs = verify_all(broken_yaml_unused, repo, readme_citing_facts)
        if f != 1 or "BROKEN" not in msgs[0]:
            print(
    ...
    # Codex Pass 3 Gap 1: smoke and production binders shared regex
    ...
    # Case 8b: production README mentions "26,947 facts in the
    ...
    production_only_readme = (
        "Our live production corpus contains 26,947 facts on the production "
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        broken_caught = any(
            "production corpus facts" in m and "BROKEN" in m for m in msgs
        )
        if not broken_caught:
            ...
            print(
                "PASS test 8b: production '26,947 facts' citation hard-failed "
    ...
    # 14-table preserve schema). Both rows are non-broken sql_query
    ...
    #   - bind the production 14-table row to the production citation
    #     only,
    ...
    smoke_and_prod_schema = [
        ...
        {
            "claim": "14-table preserve schema",
            "source": "pg_tables",
            "source_type": "sql_query",
            "expected": 14,
            "tolerance": 0,
    ...
    coexistence_readme = (
        ...
        "deployment ships a 14-table preserve schema after BL-12 lands."
    )
    with tempfile.TemporaryDirectory() as tmp:
        repo = Path(tmp)
        p, f, msgs = verify_all(
        ...
        if f != 0 or p != 2:
            ...
            print(
                "PASS test 8c: smoke 13-table and production 14-table "
    ...
    # schema" / "14-table preserve schema" hyphenation; the bare
    ...
    # 9b: README uses "14-table preserve schema" — should bind to 14.
    readme_14 = "After 008 the count rises to a 14-table preserve schema."
    with tempfile.TemporaryDirectory() as tmp:
        repo = Path(tmp)
        p, f, msgs = verify_all(hyphen_yaml_14, repo, readme_14)
        if f != 0 or p != 1:
            print(
                f"FAIL test 9b: '14-table preserve schema' phrasing not "
                f"matched, got pass={p} fail={f}",
                file=sys.stderr,
            )
            for m in msgs:
            ...
            print(
                "PASS test 9b: hyphenated '14-table preserve schema' bound "
                "to literal expected=14"
            )

    # 9c: README LIES — says "99-table preserve schema" while YAML
    # expects 14. Must FAIL HARD on README-vs-source disagreement.
    lying_hyphen_readme = "We allegedly have a 99-table preserve schema."
    with tempfile.TemporaryDirectory() as tmp:
        repo = Path(tmp)
        p, f, msgs = verify_all(hyphen_yaml_14, repo, lying_hyphen_readme)
        if f != 1 or not any("disagree" in m for m in msgs):
            ...
            print(
                ...
                "'99-table preserve schema' (expected 14)"
            )
    ...
    # Codex Pass 3 Gap 4: the YAML header documented `literal` but the
    ...
    # Codex Pass 3 Gap 2: numeric sql_query / file_lines rows were
    ...
    # directory with 7 *-parser.ts files and a README that cites
    # "7 deterministic parsers". Then build a lying README that says
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Honest README cites 7 — must PASS.
        honest_readme = "BrainCore ships 7 deterministic parsers."
        ...
        if f != 0 or p != 1:
            ...
            print("PASS test 11c: file_lines row README-bound to count=7")
        ...
        if f != 1 or not any("disagree" in m for m in msgs):
            ...
            print(
                ...
                "'99 deterministic parsers' (count=7)"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Path to README.md (default: <repo_root>/README.md)",
    )
    parser.add_argument(
        "--claims",
        type=Path,
    ...
    print(f"Found {len(readme_claims)} numeric claim(s) in {args.readme}")
