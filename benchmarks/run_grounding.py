...
---------------------
The plan (calm-skipping-sedgewick.md §4.7) originally suggested running
``bun src/cli.ts eval --run --subset smoke`` and reading the latest row
of ``preserve.eval_run``. Stream D verified this path against the live
repo at ``/srv/tools/BrainCore/src/cli.ts:265-319`` and found:
...
``preserve.eval_run`` row. See plan §4.7 for the alternate schema.
"""
