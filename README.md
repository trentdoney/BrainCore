...
The fastest honest way to try BrainCore on a fresh clone is:
...
| Production corpus | P50 latency | 71.6 ms | `benchmarks/results/2026-04-09-retrieval-production.json` |
| Production corpus | P95 latency | 85.2 ms | `benchmarks/results/2026-04-09-retrieval-production.json` |
| Production corpus | evidence grounding | 98.52% | `benchmarks/results/2026-04-09-grounding-production.json` |
...
BrainCore currently ships `7 deterministic parsers`.
...
| `codex-shared-parser.ts` | CODEX_SHARED trees | shared session artifacts |
...
| `pai-parser.ts` | PAI memory markdown | curated memory and reference facts |
...
The example server exposes the `memory-search` tool and nothing else.
That is deliberate. If your deployment needs state-at-time, timelines,
explanations, embeddings, or project-aware tools, build them on the
same retrieval library in your own server rather than pretending they
ship here already.
...
| `extract --pai-memory` | process PAI memory markdown |
| `extract --codex-history` | ingest Codex history/session data |
| `extract --codex-shared` | ingest CODEX_SHARED content |
...
|---|---|---|
| Mem0 | conversational memory and fast hosted adoption | flatter memory objects, less emphasis on provenance and temporal validity |
| Hindsight | hierarchical memory and reflection loops | younger ecosystem, heavier model dependence |
| Cognee | graph-first knowledge extraction | more infrastructure and ontology overhead |
| GraphRAG | large-corpus summarization | optimized for a different scale and query shape |
| generic chat-memory layers | agent continuity | not built for incident timelines, evidence anchors, or operational playbooks |
